...
abstract contract XCaliburAdapter is IRouter {
    ...
    function quoteAddLiquidity(
        ...
        address _pair = ISwapFactory(xcalFactory).getPair(tokenA, tokenB, stable);
    ...
    function _addLiquidity(
        ...
        address _pair = ISwapFactory(xcalFactory).getPair(tokenA, tokenB, stable);
        if (_pair == address(0)) {
            _pair = ISwapFactory(xcalFactory).createPair(tokenA, tokenB, stable);
        }
        (uint reserveA, uint reserveB) = getReserves(tokenA, tokenB, stable);
        if (reserveA == 0 && reserveB == 0) {
            (amountA, amountB) = (amountADesired, amountBDesired);
    ...
    function addLiquidityETH(
        ...
        address pair = pairFor(token, address(xcalWeth), stable);
        _safeTransferFrom(token, msg.sender, pair, amountToken);
        xcalWeth.deposit{value: amountETH}();
        assert(xcalWeth.transfer(pair, amountETH));
        liquidity = ISwapPair(pair).mint(to);
    ...
    {
        require(routes[0].from == address(xcalWeth), 'BaseV1Router: INVALID_PATH');
        amounts = getAmountsOut(msg.value, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        xcalWeth.deposit{value: amounts[0]}();
        assert(xcalWeth.transfer(pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]));
        _swap(amounts, routes, to);
    }
    ...
    returns (uint[] memory amounts)
    {
        require(routes[routes.length - 1].to == address(xcalWeth), 'BaseV1Router: INVALID_PATH');
        amounts = getAmountsOut(amountIn, routes);
        require(amounts[amounts.length - 1] >= amountOutMin, 'BaseV1Router: INSUFFICIENT_OUTPUT_AMOUNT');
        _safeTransferFrom(
            routes[0].from, msg.sender, pairFor(routes[0].from, routes[0].to, routes[0].stable), amounts[0]
        ...
        xcalWeth.withdraw(amounts[amounts.length - 1]);
