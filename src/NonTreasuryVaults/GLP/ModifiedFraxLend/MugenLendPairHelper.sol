...
import "fraxlend/src/contracts/interfaces/IFraxlendPair.sol";
...
contract FraxlendPairHelper {
        ...
        returns (ImmutablesAddressBool memory)
    {
        ...
        return
            ImmutablesAddressBool({
                ...
                _borrowerWhitelistActive: _fraxlendPair
        ...
        returns (uint256 _exchangeRate)
    {
        ...
        if (_oracleMultiply != address(0)) {
            (, int256 _answer, , , ) = AggregatorV3Interface(_oracleMultiply)
                .latestRoundData();
            if (_answer <= 0) {
        ...
        if (_oracleDivide != address(0)) {
            (, int256 _answer, , , ) = AggregatorV3Interface(_oracleDivide)
                .latestRoundData();
            if (_answer <= 0) {
    ...
    function previewRateInterest(
        ...
        IFraxlendPair _fraxlendPair = IFraxlendPair(_fraxlendPairAddress);
        (, , uint256 _UTIL_PREC, , , uint64 _DEFAULT_INT, , ) = _fraxlendPair
            .getConstants();
        ...
        VaultAccount memory _totalBorrow;
        {
            (
                uint128 _totalAssetAmount,
                uint128 _totalAssetShares
            ) = _fraxlendPair.totalAsset();
            _totalAsset = VaultAccount({
                amount: _totalAssetAmount,
                shares: _totalAssetShares
            });
            (
                uint128 _totalBorrowAmount,
                uint128 _totalBorrowShares
            ) = _fraxlendPair.totalBorrow();
            _totalBorrow = VaultAccount({
                amount: _totalBorrowAmount,
                shares: _totalBorrowShares
            });
        }
        ...
        if (_totalBorrow.shares == 0 || _fraxlendPair.paused()) {
            ...
            if (_isPastMaturity(_fraxlendPair.maturityDate(), _timestamp)) {
                ...
                _newRate = IRateCalculator(_fraxlendPair.rateContract())
                    .getNewRate(
                        abi.encode(
                            _currentRateInfo.ratePerSec,
                            _timestamp - _currentRateInfo.lastTimestamp,
                            (_totalBorrow.amount * _UTIL_PREC) /
                                _totalAsset.amount,
    ...
    {
        (_interestEarned, _newRate) = previewRateInterest(
        ...
        (uint128 _totalAssetAmount, uint128 _totalAssetShares) = _fraxlendPair
            .totalAsset();
        if (_feeToProtocolRate > 0) {
            _feesAmount =
                (_interestEarned * _feeToProtocolRate) /
                _FEE_PRECISION;
            _feesShare =
                (_feesAmount * _totalAssetShares) /
        ...
        )
    {
        ...
        {
            uint256 _exchangeRate = previewUpdateExchangeRate(
                _fraxlendPairAddress
            );
            _borrowerShares = _fraxlendPair
            ...
            uint256 _userCollateralBalance = _fraxlendPair
            ...
            uint256 _liquidationAmountInCollateralUnits = ((_totalBorrow
                .toAmount(_borrowerShares, false) * _exchangeRate) /
                _EXCHANGE_PRECISION);
        ...
        if (
            _leftoverCollateral <= 0 &&
            (_borrowerShares - _sharesToLiquidate) > 0
        ) {
            // Socialize bad debt
            _sharesToSocialize = _borrowerShares - _sharesToLiquidate;
            _amountToSocialize = (
                _totalBorrow.toAmount(_sharesToSocialize, false)
            ).toUint128();
        }
    }
}
